...
class WaterProblemReportScreen extends StatefulWidget {
  static const String screenRoute = '/water-problem-report';
  final String serviceName;
  final Color serviceColor;
  final List<Color> serviceGradient;
  final String serviceTitle;

  const WaterProblemReportScreen({
    super.key,
  ...
  @override
  State<WaterProblemReportScreen> createState() =>
      _WaterProblemReportScreenState();
}

class _WaterProblemReportScreenState extends State<WaterProblemReportScreen> {
  ...
  Widget _buildProblemReportContent() {
    IconData serviceIcon = Icons.water_drop; // ثابت للمياه
    Color iconColor = Colors.blue; // ثابت للمياه
    Color cardColor = Colors.blue; // ثابت للمياه
  ...
  void _showEmergencyReport() {
    ...
    if (widget.serviceTitle.contains('الماء')) {
      emergencyTypes = [
        'تسرب مياه خطير في الشارع',
        'أنابيب مياه مكسورة تسبب فيضان',
        'مياه جارية بغزارة من محول رئيسي',
        'خطر غرق أو انهيار بسبب المياه',
        'أخرى (أدخل في الوصف)',
      ];
    }
    ...
    String getInstructions() {
      if (widget.serviceTitle.contains('الماء')) {
        return 'يمكنك الإبلاغ عن:\n\n'
            '• تسرب مياه خطير من الأنابيب في الشارع\n'
            '• أنابيب مياه مكسورة تسبب فيضانات\n'
            '• مياه جارية بغزارة من محول رئيسي\n'
            '• خطر غرق أو انهيار بسبب المياه';
        ...
        return 'يمكنك الإبلاغ عن:\n\n'
            '• نفايات خطرة أو طبية متسربة\n'
            '• حاويات نفايات تحترق\n'
            '• تراكم نفايات يسبب خطر صحية\n'
            '• أي مشكلة خطيرة في النظافة العامة';
      }
      return 'يمكنك الإبلاغ عن أي مشكلة طارئة تتطلب تدخل سريع';
    }
                    ...
                    child: Column(
                      ...
                      children: [
                                  ...
                                  if (formKey.currentState!.validate()) {
                                    if (selectedImage == null) {
                                      ScaffoldMessenger.of(
                                        ...
                                        const SnackBar(
                                          content: Text(
                                            'يرجى التقاط صورة للمشكلة الطارئة',
                                          ),
                                        ),
                                      );
                                      return;
                        ...
                        Container(
                          ...
                          child: const Text(
                            'ملاحظة: سيتم إرسال البلاغ الطارئ إلى الجهة المختصة للمعالجة الفورية',
                            style: TextStyle(
                              color: Colors.red,
                              fontSize: 12,
                              height: 1.5,
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 16),
              const Text(
                'شكراً لك على إبلاغنا بالمشكلة الطارئة. تم إرسال بلاغك إلى فريق الطوارئ للمعالجة الفورية.',
                textAlign: TextAlign.center,
                style: TextStyle(fontSize: 16),
              ),
              const SizedBox(height: 16),
      ...
      builder: (context) => Dialog(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            crossAxisAlignment: CrossAxisAlignment.stretch,
            children: [
              Icon(Icons.check_circle, color: serviceColor, size: 60),
              const SizedBox(height: 16),
              Text(
